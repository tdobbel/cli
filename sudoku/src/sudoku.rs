...
impl Sudoku {
    ...
    pub fn min_entropy_cell(&self) -> Option<(usize, usize)> {
        ...
        for i in 0..9 {
            for j in 0..9 {
                ...
                let entro = self.possible[i][j].iter().filter(|&x| *x).count();
                if entro < vmin {
                    vmin = entro;
                    res = Some((i, j));
                }
    ...
    pub fn remove_entry(&mut self, i: usize, j: usize) -> u8 {
        ...
        for k in 0..9 {
            self.possible[i][k][num_indx] = self.ispossible(i, k, num);
            self.possible[k][j][num_indx] = self.ispossible(k, j, num);
        }
        let i0 = 3 * (i / 3);
        let j0 = 3 * (j / 3);
        for ik in 0..3 {
            for jk in 0..3 {
                self.possible[i0 + ik][j0 + jk][num_indx] = self.ispossible(i0 + ik, j0 + jk, num);
            }
        }
        num
    }
    ...
    pub fn set_entry(&mut self, i: usize, j: usize, num: u8) {
        ...
        let num_indx = (num - 1) as usize;
        for k in 0..9 {
            self.possible[i][k][num_indx] = false;
            self.possible[k][j][num_indx] = false;
        }
        let i0 = 3 * (i / 3);
        let j0 = 3 * (j / 3);
        for ik in 0..3 {
            for jk in 0..3 {
                self.possible[i0 + ik][j0 + jk][num_indx] = false;
            }
        }
    }
